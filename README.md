...
| `throttleCommits`          |              false               |                                                    true/false                                                     | When enabled, throttles commit requests by delaying them 500ms, allowing rapid changes to be batched into a single commit. Only works when `useAsynchronousCommits=true`. Cannot be used with synchronous commits.                                                                                                                                                                                                                                      |
...
| `renderCommonCommitFields` |              false               |                                                    true/false                                                     | Determines whether the API renders a structured commit object with `successStatus`, `completionStatus`, `totalTimeSeconds`, `score`, and `runtimeData`. SCORM 2004 structured commits also include the sequencing `globalObjectives` snapshot.                                                                                                                                                                                                           |
...
| `accumulateSessionTimeOnTerminate` |              false               |                                                    true/false                                                     | After a successful SCORM 2004 Terminate, fold the completed session into `cmi.total_time` on the API instance so a later read or commit reports the cumulative total. Always on for sequenced courses; off by default elsewhere. |
...
| `globalObjectiveIds`       |                []                |                                                     string[]                                                      | (SCORM 2004) Host-defined objective IDs exposed directly as global CMI rows. Manifest `<imsss:mapInfo>` targets are discovered from the sequencing activity tree and should not be duplicated here.                                                                                                                                                                                                                                                         |
...
| `sequencingStatePersistence` |           undefined             |                                      SequencingStatePersistenceConfig                                             | (SCORM 2004) Persistence callbacks and options for activity tracking state. Set `autoLoadOnInitialize: false` when the LMS explicitly awaits `loadSequencingState()` before initial delivery.                                                                                                                                                                                                                                                                |
...
3. **Data Persistence**: The commit data from sequenced courses includes additional metadata that the LMS should store for proper sequencing support.
   ...
   With `renderCommonCommitFields: true`, SCORM 2004 commit objects include a `globalObjectives` snapshot. Restore it with `restoreGlobalObjectiveSnapshot()` before initial delivery. If the LMS explicitly awaits `loadSequencingState()`, configure `sequencingStatePersistence.autoLoadOnInitialize` as `false` to prevent a second asynchronous load during `Initialize()`.
