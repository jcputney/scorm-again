...
export class TerminationHandler {
   ...
   */
  private transferRteData(activity: Activity): void {
    if (!this.getCMIData) {
  ...
  private transferNonPrimaryObjectives(
    ...
    for (const cmiObjective of cmiData.objectives) {
      ...
      // Transfer completion status
      if (
        cmiObjective.completion_status &&
        cmiObjective.completion_status !== "unknown"
      ) {
        activityObjective.completionStatus =
          cmiObjective.completion_status as CompletionStatus;
      }
      ...
      if (hasSuccessStatus || hasNormalizedMeasure) {
        const finalStatus = hasSuccessStatus
          ? successStatus
          : activityObjective.satisfiedStatus;
        const finalMeasure = hasNormalizedMeasure
          ? normalizedScore
