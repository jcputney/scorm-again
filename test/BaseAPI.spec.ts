...
import { RefObject, ResultObject, Settings } from "../src/types/api_types";
...
class TestAPI extends BaseAPI {
  ...
  lmsGetValue(_CMIElement: string) { return ""; }
  lmsSetValue(_CMIElement: string, _value: any) { return "true"; }
  lmsCommit() { return "true"; }
  lmsGetLastError() { return "0"; }
  lmsGetErrorString(_CMIErrorCode: string | number) { return ""; }
  lmsGetDiagnostic(_CMIErrorCode: string | number) { return ""; }
  validateCorrectResponse(_CMIElement: string, _value: any) {}
  getChildElement(_CMIElement: string, _value: any, _foundFirstIndex: boolean) { return null; }
  ...
  renderCommitObject(_terminateCommit: boolean) {
    return {
  ...
  public exposedGetLmsErrorMessageDetails(errorNumber: string | number, detail: boolean = false): string {
...
describe("BaseAPI", () => {
  ...
  describe("renderCMIToJSONString", () => {
    it("should call serializationService.renderCMIToJSONString with cmi and sendFullCommit", () => {
      // Arrange
      const renderCMIToJSONStringStub = sinon.stub(api["_serializationService"], "renderCMIToJSONString").returns("{}");
  ...
  describe("renderCMIToJSONObject", () => {
    it("should call serializationService.renderCMIToJSONObject with cmi and sendFullCommit", () => {
      // Arrange
      const renderCMIToJSONObjectStub = sinon.stub(api["_serializationService"], "renderCMIToJSONObject").returns({});
  ...
  describe("initialize", () => {
    it("should return SCORM_FALSE when already initialized", () => {
      ...
      expect(throwSCORMErrorSpy.calledWith(errorCodes.INITIALIZED, "Already initialized")).toBe(true);
    });
    ...
    it("should return SCORM_FALSE when already terminated", () => {
      ...
      // Assert
      expect(result).toBe(global_constants.SCORM_FALSE);
      expect(throwSCORMErrorSpy.calledWith(errorCodes.TERMINATED, "Already terminated")).toBe(true);
    });
  ...
  describe("loadFromFlattenedJSON", () => {
    it("should call serializationService.loadFromFlattenedJSON with correct parameters", () => {
      // Arrange
      const loadFromFlattenedJSONStub = sinon.stub(api["_serializationService"], "loadFromFlattenedJSON");
      const json = { "cmi.core.student_id": "123" };
