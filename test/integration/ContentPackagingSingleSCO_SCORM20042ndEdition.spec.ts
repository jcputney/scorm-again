 ...
 *
 * This module is a single SCO module for SCORM 2004 2nd Edition. It differs from 3rd Edition in that:
 * - Manifest metadata uses "CAM 1.3" instead of "2004 3rd Edition"
 ...
 * - Edition-specific verification (2nd edition metadata)
 */

// Module path - single SCO, can load any content page directly
// Using Playing.html as the default entry point
const MODULE_PATH =
  "/test/integration/modules/ContentPackagingSingleSCO_SCORM20042ndEdition/Playing/Playing.html";
...
wrappers.forEach((wrapper) => {
  test.describe(`ContentPackagingSingleSCO SCORM 2004 2nd Edition Integration (${wrapper.name})`, () => {
    ...
    test("should verify SCORM 2004 2nd Edition metadata", async ({ page }) => {
      // This test verifies we're working with a 2nd edition module
      // The manifest should specify "CAM 1.3" as the schemaversion
      const contentPath =
        "/test/integration/modules/ContentPackagingSingleSCO_SCORM20042ndEdition/Playing/Playing.html";
    ...
    test("should load and interact with module content pages", async ({ page }) => {
      ...
      const contentPath =
        "/test/integration/modules/ContentPackagingSingleSCO_SCORM20042ndEdition/Etiquette/Course.html";
      await page.goto(`${wrapper.path}?module=${contentPath}`);
      await page.waitForLoadState("networkidle");
    ...
    test("should load and take a quiz, recording interactions and scores", async ({ page }) => {
      ...
      const assessmentPath =
        "/test/integration/modules/ContentPackagingSingleSCO_SCORM20042ndEdition/shared/assessmenttemplate.html?questions=Etiquette";
    ...
    test("should record individual question interactions when taking quiz", async ({ page }) => {
      ...
      const assessmentPath =
        "/test/integration/modules/ContentPackagingSingleSCO_SCORM20042ndEdition/shared/assessmenttemplate.html?questions=Playing";
      await page.goto(`${wrapper.path}?module=${assessmentPath}`);
      await page.waitForLoadState("networkidle");
    ...
    test("should handle quiz for all question sets", async ({ page }) => {
      ...
      for (const questionSet of questionSets) {
        ...
        const assessmentPath = `/test/integration/modules/ContentPackagingSingleSCO_SCORM20042ndEdition/shared/assessmenttemplate.html?questions=${questionSet}`;
        await page.goto(`${wrapper.path}?module=${assessmentPath}`);
        await page.waitForLoadState("networkidle");
