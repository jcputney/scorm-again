...
wrappers.forEach((wrapper) => {
  test.describe(`SequencingRandomTest SCORM 2004 3rd Edition Integration (${wrapper.name})`, () => {
    ...
    test("should mark post test as not satisfied when any test fails per SCORM 2004 SN Book SB.2.4", async ({
      ...
      expect(
        posttestStatus.successStatus !== "passed" || posttestStatus.completionStatus,
      ).toBeTruthy();
    });
        ...
        if (state?.rootActivity?.children) {
          ...
          if (posttest?.children) {
            return posttest.children.map((child: any) => ({
              id: child.id,
              hasExitParent:
                child.sequencingRules?.postConditionRules?.some(
     ...
     * - Once the post test has been passed or attempted twice, the learner exits the course
     */
    test("should configure exitAll behavior per SCORM 2004 SN Book SB.2.3", async ({ page }) => {
      await page.goto(`${wrapper.path}?module=${MODULE_PATH}`);
      await page.waitForLoadState("networkidle");
     ...
     */
    test("should track global objectives per SCORM 2004 SN Book SB.2.4", async ({ page }) => {
      await page.goto(`${wrapper.path}?module=${MODULE_PATH}`);
      await page.waitForLoadState("networkidle");
      ...
      expect(objectiveId).toBe("com.scorm.golfsamples.sequencing.randomtest.content_completed");
      ...
      expect(courseScoreId).toBe("com.scorm.golfsamples.sequencing.randomtest.course_score");
    });
