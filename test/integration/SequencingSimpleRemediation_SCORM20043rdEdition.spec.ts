          ...
          objectives: [
            {
              ...
              mapInfo: [
                {
                  targetObjectiveID: "com.scorm.golfsamples.sequencing.simpleremediation.20043rd.playing_satisfied",
          ...
          objectives: [
            {
              ...
              mapInfo: [
                {
                  targetObjectiveID: "com.scorm.golfsamples.sequencing.simpleremediation.20043rd.etiquette_satisfied",
                  readSatisfiedStatus: true,
                  writeSatisfiedStatus: false,
                },
              ],
          ...
          objectives: [
            {
              ...
              mapInfo: [
                {
                  targetObjectiveID: "com.scorm.golfsamples.sequencing.simpleremediation.20043rd.handicapping_satisfied",
                  readSatisfiedStatus: true,
                  writeSatisfiedStatus: false,
                },
              ],
          ...
          objectives: [
            {
              ...
              mapInfo: [
                {
                  targetObjectiveID: "com.scorm.golfsamples.sequencing.simpleremediation.20043rd.havingfun_satisfied",
                  readSatisfiedStatus: true,
                  writeSatisfiedStatus: false,
                },
              ],
...
wrappers.forEach((wrapper) => {
  test.describe(
    `SequencingSimpleRemediation SCORM 2004 3rd Edition Integration (${wrapper.name})`,
    () => {
      ...
      test("should configure flow-only navigation per SCORM 2004 SN Book SB.2.1", async ({
        ...
        const controls = await page.evaluate(() => {
          ...
          return {
            ...
            wrapperChoiceExit: state?.rootActivity?.children?.[0]?.sequencingControls?.choiceExit ?? false,
          };
        });
          ...
          if (state?.rootActivity?.children?.[0]?.children) {
            ...
            if (test1?.objectives?.[0]?.mapInfo?.[0]) {
              return {
                ...
                writeNormalizedMeasure: test1.objectives[0].mapInfo[0].writeNormalizedMeasure ?? false,
              };
            }
          }
          return null;
          ...
          if (state?.rootActivity?.children?.[0]?.children) {
            ...
            if (test4?.sequencingRules?.postConditionRules) {
              return test4.sequencingRules.postConditionRules.find(
                (rule: any) => rule.action === "exitParent" || rule.action === "exit_parent" || rule.action?.toLowerCase() === "exitparent"
              );
            }
          }
          return null;
      ...
      test("should configure content objects with no rollup per SCORM 2004 SN Book SB.2.4", async ({
        ...
        const contentRollup = await page.evaluate(() => {
          ...
          if (state?.rootActivity?.children?.[0]?.children) {
            ...
            return {
              rollupObjectiveSatisfied: playing?.sequencingControls?.rollupObjectiveSatisfied ?? false,
              rollupProgressCompletion: playing?.sequencingControls?.rollupProgressCompletion ?? false,
      ...
      test("should determine navigation validity for all activities per SCORM 2004 SN Book SB.2.2", async ({
        ...
        const navValidity = await page.evaluate(() => {
          const api = (window as any).API_1484_11;
          const activities = ["playing_item", "etuqiette_item", "handicapping_item", "havingfun_item", "test_1", "test_2", "test_3", "test_4"];
      ...
      test("should preserve global objectives after API reset per SCORM 2004 SN Book SB.2.4", async ({
        ...
        const globalObjectiveId = "com.scorm.golfsamples.sequencing.simpleremediation.20043rd.playing_satisfied";
        await page.evaluate(
          ({ objectiveId }) => {
            const api = (window as any).API_1484_11;
