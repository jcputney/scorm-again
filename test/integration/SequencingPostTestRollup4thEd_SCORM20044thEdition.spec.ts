...
wrappers.forEach((wrapper) => {
  test.describe(
    `SequencingPostTestRollup4thEd SCORM 2004 4th Edition Integration (${wrapper.name})`,
    () => {
       ...
       */
      test("should support progress measure rollup per SCORM 2004 4th Edition", async ({
        page,
      }) => {
        await page.goto(`${wrapper.path}?module=${MODULE_PATH}`);
        await page.waitForLoadState("networkidle");
       ...
       */
      test("should support shared data buckets per SCORM 2004 4th Edition", async ({
        page,
      }) => {
        await page.goto(`${wrapper.path}?module=${MODULE_PATH}`);
        await page.waitForLoadState("networkidle");
      ...
      test("should preserve global objectives after API reset per SCORM 2004 SN Book SB.2.4 (4th Edition)", async ({
        ...
        const globalObjectiveId = "com.scorm.golfsamples.sequencing.forcedsequential.playing_completed";
        await page.evaluate(
          ({ objectiveId }) => {
            const api = (window as any).API_1484_11;
