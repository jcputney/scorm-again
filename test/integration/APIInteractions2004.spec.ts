...
test.describe("SCORM 2004 API Interactions", () => {
  test("should handle basic data model interactions correctly", async ({ page }) => {
    ...
    expect(testInteractions.errorHandling.invalidElement.diagnosticInfo.length).toBeGreaterThan(0);
    ...
    expect(testInteractions.errorHandling.readOnlyElement.diagnosticInfo.length).toBeGreaterThan(0);
  ...
  test("should handle advanced data model interactions", async ({ page }) => {
    ...
    const advancedInteractions = await page.evaluate(() => {
      ...
      const interactionResults = {
        ...
        createInteraction: window.API_1484_11.lmsSetValue("cmi.interactions.0.id", "interaction_1"),
        ...
        getLearnerResponse: window.API_1484_11.lmsGetValue("cmi.interactions.0.learner_response"),
        getCorrectResponse: window.API_1484_11.lmsGetValue(
          "cmi.interactions.0.correct_responses.0.pattern",
        ),
        getObjective: window.API_1484_11.lmsGetValue("cmi.interactions.0.objectives.0.id"),
      };
      ...
      const objectiveResults = {
        ...
        setScore: window.API_1484_11.lmsSetValue("cmi.objectives.0.score.scaled", "0.75"),
        setSuccess: window.API_1484_11.lmsSetValue("cmi.objectives.0.success_status", "passed"),
        ...
        getScore: window.API_1484_11.lmsGetValue("cmi.objectives.0.score.scaled"),
        getSuccess: window.API_1484_11.lmsGetValue("cmi.objectives.0.success_status"),
        getCompletion: window.API_1484_11.lmsGetValue("cmi.objectives.0.completion_status"),
        getDescription: window.API_1484_11.lmsGetValue("cmi.objectives.0.description"),
      };
    ...
    expect(advancedInteractions.interactionResults.getLearnerResponse).toBe("a[,]b");
    expect(advancedInteractions.interactionResults.getCorrectResponse).toBe("a[,]b");
    expect(advancedInteractions.interactionResults.getObjective).toBe("objective_1");
    ...
    expect(advancedInteractions.objectiveResults.getCompletion).toBe("completed");
    expect(advancedInteractions.objectiveResults.getDescription).toBe("Test objective");
