...
describe("CrossFrameAPI", () => {
  ...
  beforeEach(() => {
    ...
    syncStub = sinon.stub(client as any, "_syncCall");
    sendStub = sinon.stub(client as any, "_sendMessage");
  ...
  [
    ...
    it(`should call ${method} and return value from _syncCall`, () => {
      syncStub.withArgs(syncName, sinon.match.any).returns(success);
      const result = client[method](...args);
      expect(result).toBe(success);
    });
    it(`should handle error in ${method} and set _lastError`, () => {
      syncStub.withArgs(syncName, sinon.match.any).throws(new Error("fail"));
      const result = client[method](...args);
  ...
  it("should trigger event listeners on event", () => {
    ...
    client._handleEvent("CustomEvent", "arg1", 5);
    expect(cb.calledOnce).toBe(true);
    expect(cb.calledWith("arg1", 5)).toBe(true);
  });
  it("should not trigger after off", () => {
    const cb = sinon.spy();
    client.on("Ev", cb);
    client.off("Ev", cb);
    client._handleEvent("Ev", 1, 2);
    expect(cb.called).toBe(false);
  });
  it('should trigger "*" listeners for any event', () => {
    const cb = sinon.spy();
    client.on("*", cb);
    client._handleEvent("SomeEvent", "foo", "bar");
    expect(cb.calledOnce).toBe(true);
    expect(cb.calledWith("SomeEvent", "foo", "bar")).toBe(true);
  });
});
...
describe("CrossFrameLMS", () => {
  ...
  beforeEach(() => {
    api = {
      lmsGetValue: sinon.stub().returns("abc"),
      setValue: sinon.stub().returns(true),
    };
  ...
  it("should process message and call API method, respond with result and sab", () => {
    ...
    server._processMessage(msg, source, "http://origin");
    expect(api.lmsGetValue.calledOnceWith("cmi.score.raw")).toBe(true);
    expect(postMessageSpy.calledOnce).toBe(true);
    const resp = postMessageSpy.getCall(0).args[0];
    expect(resp.messageId).toBe("msg-1");
    expect(resp.result).toBe("abc");
    expect(resp.error).toBeUndefined();
    expect(resp.sab).toBe(sab);
    // Third arg is sab transfer
    expect(postMessageSpy.getCall(0).args[2]).toEqual([sab]);
  });
